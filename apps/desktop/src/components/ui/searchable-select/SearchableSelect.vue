...
// Self-contained options filter helper
function filterOptions(options: string[], search: string, displayName: (opt: string) => string): string[] {
  const query = search.trim().toLowerCase();
  if (!query) return options;
  return options.filter((option) => {
    return option.toLowerCase().includes(query) || displayName(option).toLowerCase().includes(query);
  });
}

const filteredOptions = computed(() => filterOptions(props.options, searchText.value, props.displayName));
